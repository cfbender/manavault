...
function normalizeGalleryCards(
  cards: readonly DomeGalleryCard[],
): GalleryCard[] {
  return cards
    .map((card) => {
...
function formatSetLabel(
  card: Pick<DomeGalleryCard, "collectorNumber" | "setCode">,
) {
...
function firstFinish(card: Pick<DomeGalleryCard, "finish" | "finishes">) {
  return (
    card.finish ||
    card.finishes?.find((finish): finish is string => Boolean(finish)) ||
    null
  );
}

function buildItems(
  pool: readonly GalleryCard[],
  segmentCount: number,
): DomeGalleryItem[] {
...
export function DomeGallery({
  ...
  const items = useMemo(
    () => buildItems(galleryCards, safeSegments),
    [galleryCards, safeSegments],
  );
  ...
  useEffect(() => {
    ...
    const resizeObserver = new ResizeObserver((entries) => {
      ...
      const maxRadiusValue = Number.isFinite(maxRadius)
        ? maxRadius
  ...
  return (
    ...
    >
      <div
        ...
        aria-label="Card art dome gallery"
      >
