    ...
    <div className="min-h-screen bg-base-100 text-base-content">
      <header className="app-shell-header sticky top-0 z-30 bg-base-100/95 backdrop-blur">
        <div className="navbar min-h-16 px-0">
          <Link to="/" className="flex min-w-0 items-center gap-3 text-2xl font-black tracking-normal">
...
export function PageSection({ title, count, children }: { title?: string; count?: ReactNode; children: ReactNode }) {
  return (
    <section className="space-y-3">
      {title || count ? (
        <div className="flex items-center justify-between gap-3">
          {title ? <h2 className="text-2xl font-black tracking-normal">{title}</h2> : <span />}
          {count ? <span className="badge border-transparent bg-base-200 text-sm">{count}</span> : null}
        </div>
      ) : null}
      {children}
    </section>
...
export function ActionCard({
  ...
  return (
    <Link to={to} className="card group h-full border border-base-300 bg-base-100 shadow-sm transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-xl">
