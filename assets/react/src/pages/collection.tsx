...
function EditLocationDialog({ location, onOpenChange, open }: { location: LocationSummary | LocationDetail | null; onOpenChange: (open: boolean) => void; open?: boolean }) {
  ...
  const coverSearchTerm = coverSearch.trim()
      ...
      <DialogContent className="max-w-3xl" labelledBy="edit-location-title">
        ...
        <form className="space-y-5 p-5" onSubmit={submit}>
          ...
          <section className="space-y-3">
            ...
            {coverSearchTerm.length > 1 ? (
              <div className="max-h-80 overflow-y-auto rounded-box border border-base-300 bg-base-100">
                {coverSearchQuery.isFetching ? <p className="px-3 py-2 text-sm text-base-content/55">Searching...</p> : null}
                {!coverSearchQuery.isFetching && coverSearchQuery.data?.cards.length === 0 ? <p className="px-3 py-2 text-sm text-base-content/55">No cards found.</p> : null}
