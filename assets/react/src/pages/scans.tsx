...
export function ScanSessionsPage() {
  const { data, isLoading } = useQuery({ queryKey: ["scan-sessions"], queryFn: () => request(ScanSessionsDocument) })
            ...
            <Link key={session.id} to="/scan-sessions/$id" params={{ id: session.id }}>
              <Card className="h-full transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:bg-base-100 hover:shadow-lg">
                ...
                <CardContent className="flex flex-wrap gap-2">
                  ...
                  <Badge tone={session.reviewCount ? "warning" : "success"}>{session.reviewCount || 0} review</Badge>
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<Button asChild><Link to="/scan-sessions/$id/scanner" params={{ id: "0" }}><Camera className="h-4 w-4" />Scanner</Link></Button>}
      />
      <EmptyState title="Review UI pending" description="The React route is mounted; scan item mutations and camera capture are the next parity slice." />
    </>
  )
}
...
export function ScannerPage() {
  return (
    <>
      <PageHeader title="Scanner" description="Camera capture will reuse the existing native/PWA camera code in the next pass." />
