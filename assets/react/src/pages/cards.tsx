...
export function CardsPage({ query }: { query: string }) {
  ...
  function updateSearchDraft(value: string) {
    setQ(value);
    if (!value.trim() && query)
      navigate({ to: "/cards", search: { q: undefined } });
  }
...
function FullscreenPrintingDialog({
  ...
  const currentIndex = currentPrintingId
    ? printings.findIndex(
        (printing) => printing.scryfallId === currentPrintingId,
      )
    : -1;
  ...
  const profileGlowColor = foil
    ? "rgba(255, 219, 122, 0.62)"
    : "rgba(125, 190, 255, 0.55)";
  const canNavigate = printings.length > 1 && currentIndex >= 0;
  const positionLabel = canNavigate
  ...
  useEffect(() => {
    ...
    function handleKeyDown(event: KeyboardEvent) {
      ...
      const nextIndex =
        (currentIndex + direction + printings.length) % printings.length;
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                {subtitle ? (
                  <p className="mt-1 line-clamp-2 text-sm text-neutral-content/65">
                    {subtitle}
                  </p>
                ) : null}
                {finish ||
            ...
            <div className="relative flex min-h-0 flex-1 items-center justify-center mb-12">
              ...
              <motion.div
                ...
                aria-label={
                  canNavigate ? `Show next printing of ${card.name}` : undefined
                }
              ...
              >
                <ProfileCard
                  ...
                  title={subtitle}
                  handle={
                    setLabel || printing.setCode?.toUpperCase() || "printing"
                  }
...
const CARD_DECK_ZONES: CardDeckZone[] = [
  "mainboard",
  "sideboard",
  "commander",
  "maybeboard",
];
const NON_COMMANDER_CARD_DECK_ZONES: CardDeckZone[] = [
  "mainboard",
  "sideboard",
  "maybeboard",
];
...
function AddCatalogCardToDeckDialog({
  ...
  const selectedDeck = decksQuery.data?.decks.find(
    (deck) => deck.id === deckId,
  );
  const zoneOptions =
    selectedDeck?.format === "commander"
      ? CARD_DECK_ZONES
      : NON_COMMANDER_CARD_DECK_ZONES;
  const finishOptions =
    target?.finishes?.length &&
    target.finishes.some((value) => CARD_DECK_FINISHES.includes(value))
  ...
  const addToDeck = useMutation({
    ...
    onError: (error) =>
      setError(
        error instanceof Error ? error.message : "Could not add card to deck",
      ),
  });
  ...
  return (
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && close()}>
      <DialogContent
        className="max-w-xl"
        labelledBy="add-catalog-card-to-deck-title"
      >
        <DialogHeader>
          <div>
            <DialogTitle id="add-catalog-card-to-deck-title">
              Add to deck
            </DialogTitle>
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            ...
            <label className="form-control">
              ...
              <select
                ...
                disabled={addToDeck.isPending}
                onChange={(event) =>
                  setZone(event.target.value as CardDeckZone)
                }
              >
...
function renderRichCardText(text: string) {
  return text
    ...
    .map((part, index) => {
      if (/^\{[^}]+\}$/.test(part))
        return <ManaSymbol key={index} symbol={part} />;
      if (/^\([^)]*\)$/.test(part)) {
        return (
