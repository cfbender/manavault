...
export function CardsPage({ query }: { query: string }) {
  ...
  return (
    <>
      <PageHeader title="Card search" description="Search the local Scryfall catalog and add exact printings to your collection." />
      <form className="control-toolbar mb-5 flex gap-2">
        <Input name="q" value={q} onChange={event => setQ(event.target.value)} placeholder="Card name" />
        <Button asChild>
          ...
          {data.cards.map(card => {
            ...
            return (
              <Link key={card.oracleId} to="/cards/$id" params={{ id: card.oracleId }} search={{ q: query }}>
                <Card className="h-full overflow-hidden transition-colors hover:bg-base-200">
        ...
        <div className="space-y-5">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {printings.filter(present).map(printing => (
              <section key={printing.scryfallId} className="space-y-3 rounded-lg border border-base-300 bg-base-100 p-4">
