      ...
      <form onSubmit={submitSettings} className="card border border-base-300 bg-base-100 shadow-sm">
        <div className="card-body gap-6 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <Field label="CRON schedule" htmlFor="backup-cron" help="Five-field CRON, evaluated once per minute. Example: 0 3 * * * backs up daily at 03:00 UTC.">
              <Input
                id="backup-cron"
                value={form.cron}
          ...
          {form.provider === "s3" ? <S3Fields form={form} setFormField={setFormField} hasSecret={settings?.hasS3SecretAccessKey ?? false} /> : null}
          {form.provider === "google_drive" ? <GoogleDriveFields form={form} setFormField={setFormField} settings={settings} /> : null}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <Button type="button" variant="outline" onClick={runBackup} disabled={runMutation.isPending || form.provider === "none"}>
        ...
        <div className="card border border-base-300 bg-base-100 shadow-sm">
          <div className="card-body gap-4 p-6">
            ...
            <div className="grid gap-3 md:grid-cols-[1fr_auto]">
              ...
              <Button type="button" variant="destructive" onClick={stageRestore} disabled={restoreMutation.isPending || !restoreId}>
                <DownloadCloud className="h-4 w-4" />
                {restoreMutation.isPending ? "Staging..." : "Stage restore"}
              </Button>
            </div>
            <Button type="button" variant="ghost" className="w-fit" onClick={() => backupsQuery.refetch()} disabled={backupsQuery.isFetching || form.provider === "none"}>
              <RefreshCw className="h-4 w-4" />
              Refresh cloud list
            </Button>
          </div>
...
function S3Fields({ form, setFormField, hasSecret }: { form: FormState; setFormField: <K extends keyof FormState>(field: K, value: FormState[K]) => void; hasSecret: boolean }) {
  return (
    <div className="grid gap-4 rounded-box border border-base-300 bg-base-200/50 p-4 md:grid-cols-2">
      <Field label="Endpoint" htmlFor="s3-endpoint" help="R2 example: https://<account>.r2.cloudflarestorage.com">
        <Input id="s3-endpoint" value={form.s3Endpoint} onChange={(event) => setFormField("s3Endpoint", event.target.value)} placeholder="https://..." />
      </Field>
      <Field label="Bucket" htmlFor="s3-bucket">
        <Input id="s3-bucket" value={form.s3Bucket} onChange={(event) => setFormField("s3Bucket", event.target.value)} />
      </Field>
      <Field label="Region" htmlFor="s3-region" help="Use auto for Cloudflare R2.">
        <Input id="s3-region" value={form.s3Region} onChange={(event) => setFormField("s3Region", event.target.value)} />
      </Field>
      <Field label="Prefix" htmlFor="s3-prefix">
        <Input id="s3-prefix" value={form.s3Prefix} onChange={(event) => setFormField("s3Prefix", event.target.value)} placeholder="manavault" />
      </Field>
      <Field label="Access key ID" htmlFor="s3-access-key">
        <Input id="s3-access-key" value={form.s3AccessKeyId} onChange={(event) => setFormField("s3AccessKeyId", event.target.value)} />
      </Field>
      <Field label="Secret access key" htmlFor="s3-secret" help={hasSecret ? "Leave blank to keep the saved secret." : undefined}>
        <Input id="s3-secret" type="password" value={form.s3SecretAccessKey} onChange={(event) => setFormField("s3SecretAccessKey", event.target.value)} placeholder={hasSecret ? "Saved" : ""} />
      </Field>
    </div>
  )
}

function GoogleDriveFields({ form, setFormField, settings }: { form: FormState; setFormField: <K extends keyof FormState>(field: K, value: FormState[K]) => void; settings?: { hasGoogleClientSecret?: boolean | null; hasGoogleRefreshToken?: boolean | null } | null }) {
  return (
    <div className="grid gap-4 rounded-box border border-base-300 bg-base-200/50 p-4 md:grid-cols-2">
      <Field label="OAuth client ID" htmlFor="google-client-id">
        <Input id="google-client-id" value={form.googleClientId} onChange={(event) => setFormField("googleClientId", event.target.value)} />
      </Field>
      <Field label="OAuth client secret" htmlFor="google-client-secret" help={settings?.hasGoogleClientSecret ? "Leave blank to keep the saved secret." : undefined}>
        <Input id="google-client-secret" type="password" value={form.googleClientSecret} onChange={(event) => setFormField("googleClientSecret", event.target.value)} placeholder={settings?.hasGoogleClientSecret ? "Saved" : ""} />
      </Field>
      <Field label="Refresh token" htmlFor="google-refresh-token" help={settings?.hasGoogleRefreshToken ? "Leave blank to keep the saved token." : "Use a token with Google Drive file access."}>
        <Input id="google-refresh-token" type="password" value={form.googleRefreshToken} onChange={(event) => setFormField("googleRefreshToken", event.target.value)} placeholder={settings?.hasGoogleRefreshToken ? "Saved" : ""} />
      </Field>
      <Field label="Folder ID" htmlFor="google-folder-id" help="Optional. Backups go to My Drive if blank.">
        <Input id="google-folder-id" value={form.googleFolderId} onChange={(event) => setFormField("googleFolderId", event.target.value)} />
      </Field>
    </div>
  )
}

function StatusSummary({ settings }: { settings: { lastBackupAt?: string | null; lastBackupStatus?: string | null; lastBackupMessage?: string | null; lastRestoreAt?: string | null; lastRestoreStatus?: string | null; lastRestoreMessage?: string | null } | null | undefined }) {
  if (!settings) return null

  return (
    <div className="grid gap-3 border-t border-base-300 pt-4 text-sm md:grid-cols-2">
      <StatusLine label="Last backup" at={settings.lastBackupAt} status={settings.lastBackupStatus} message={settings.lastBackupMessage} />
      <StatusLine label="Last restore" at={settings.lastRestoreAt} status={settings.lastRestoreStatus} message={settings.lastRestoreMessage} />
    </div>
  )
}

function StatusLine({ label, at, status, message }: { label: string; at?: string | null; status?: string | null; message?: string | null }) {
  return (
    <div className="rounded-box border border-base-300 bg-base-200/40 p-3">
      <div className="font-bold">{label}</div>
      <div className="text-base-content/70">{status ? `${status}${at ? ` · ${formatDate(at)}` : ""}` : "No activity yet"}</div>
...
function Alert({ tone, children }: { tone: "success" | "error"; children: ReactNode }) {
  return <div className={`alert ${tone === "success" ? "alert-success" : "alert-error"}`}>{children}</div>
}
