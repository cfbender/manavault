...
import {
  DisableTradeWantsSharingDocument,
  EnsureTradeWantsShareTokenDocument,
  RotateTradeWantsShareTokenDocument,
} from "./documents"

export function WantsShareDialog({
  onOpenChange,
  open,
}: {
  ...
  const [ensureShareToken, ensureShare] = useMutation(EnsureTradeWantsShareTokenDocument)
  const [rotateShareToken, rotateShare] = useMutation(RotateTradeWantsShareTokenDocument)
  const [disableSharing, disableShare] = useMutation(DisableTradeWantsSharingDocument)

  const token = managedToken ?? ""
  const shareUrl =
    token && typeof window !== "undefined"
      ? `${window.location.origin}/share/wants/${encodeURIComponent(token)}`
      : ""
  ...
  useEffect(() => {
    ...
    void ensureShareToken({
      onCompleted: (data) => setManagedToken(data.ensureTradeWantsShareToken?.token ?? ""),
    })
  ...
  function rotateLink() {
    void rotateShareToken({
      onCompleted: (data) => {
        setManagedToken(data.rotateTradeWantsShareToken?.token ?? "")
        setCopyState("idle")
        showToast("Wants link rotated")
      },
    })
  }
  ...
  function disableLink() {
    ...
    void disableSharing({
      onCompleted: () => {
        setManagedToken("")
        showToast("Wants sharing disabled")
        onOpenChange(false)
      },
    })
  }
  ...
  return (
    <>
      ...
      >
        <DialogContent className="max-w-xl" labelledBy="share-wants-title">
          <DialogHeader>
            <div>
              <DialogTitle id="share-wants-title">Share wants</DialogTitle>
              <p className="mt-1 text-sm text-base-content/60">
                Anyone with this link can see your want list.
              </p>
            </div>
      ...
      <ConfirmDialog
        ...
        title={confirmAction === "disable" ? "Disable wants sharing?" : "Rotate wants link?"}
      >
