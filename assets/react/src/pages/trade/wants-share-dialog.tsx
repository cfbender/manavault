...
import { EnsureTradeWantsShareTokenDocument, TradeWantsShareTokenDocument } from "./documents"
...
export function WantsShareDialog({
  ...
  const tokenQuery = useQuery(TradeWantsShareTokenDocument, {
    skip: !open,
    fetchPolicy: "cache-and-network",
  })
  const [ensureShareToken, ensureShare] = useMutation(EnsureTradeWantsShareTokenDocument)

  const existingToken = tokenQuery.data?.tradeWantsShareToken || ""
  const generatedToken = ensureShare.data?.ensureTradeWantsShareToken?.token || ""
